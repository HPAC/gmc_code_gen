...
void cblas_ddisymm(const CBLAS_LAYOUT Layout, const CBLAS_SIDE Side,
  ...
  if (Layout == CblasColMajor) {
    if (Side == CblasLeft)
      ...
      // cblas_xerbla(2, "cblas_ddisymm", "Illegal Side setting, %d\n", Side);
      return;
    }
    ...
    if (UploB == CblasUpper)
      ...
      // cblas_xerbla(3, "cblas_ddisymm", "Illegal UploB setting, %d\n", UploB);
      return;
    }

    ddisymm(SD, ULB, M, alpha, A, IncA, B, ldB);

  } else if (Layout == CblasRowMajor) {
    // @todo
  } else {
    // cblas_xerbla(1, "cblas_ddisymm", "Illegal Layout setting, %d\n", Layout);
  }
}