...
void cblas_dsydisv(const CBLAS_LAYOUT Layout, const CBLAS_SIDE Side,
  ...
  if (Layout == CblasColMajor) {
    if (Side == CblasRight)
      ...
      // cblas_xerbla(2, "cblas_dsydisv", "Illegal Side setting, %d\n", Side);
      return;
    }
    ...
    if (UploA == CblasUpper)
      ...
      // cblas_xerbla(3, "cblas_dsydisv", "Illegal UploA setting, %d\n", UploA);
      return;
    }

    dsydisv(SD, ULA, M, A, ldA, D, IncD, B, ldB);

  } else if (Layout == CblasRowMajor) {
    // @todo
  } else {
    // cblas_xerbla(1, "cblas_dsydisv", "Illegal Layout setting, %d\n", Layout);
  }
}
