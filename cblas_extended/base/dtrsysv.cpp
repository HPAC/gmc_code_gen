 ...
 * if SIDE = 'L' -- op(A) * X = alpha * B;
 * otherwise     -- X * op(A) = alpha * B;
 * In every case: A and B are M by M matrices
 */
void dtrsysv(const char SIDE, const char UPLOA, const char UPLOB,
  ...
  if (INFO != 0) {
    ...
    // cblas_xerbla(INFO, "dtrsysv", "Illegal setting, %d\n");
    return;
  }
  ...
  cblas_dtrsm(CblasColMajor, TR_SIDE, UPLO_A, TRANS_A, DIAG_A, M, M, ALPHA, A,
              LDA, B, LDB);
}  // END OF DTRSYMM