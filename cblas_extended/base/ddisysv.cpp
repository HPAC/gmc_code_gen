...
void ddisysv(const char SIDE, const char UPLOB, const int M, const double ALPHA,
  ...
  if (INFO != 0) {
    ...
    // cblas_xerbla(INFO, "ddisysm", "Illegal setting, %d\n");
    return;
  }
  ...
  if (DIAG_LSIDE) {
    ...
    if (UPPER_B) {
      // B is upper-stored.
      for (int j = 0; j < M; j++) {
        B[j * LDB + j] = B[j * LDB + j] * INV_A[j];
        for (int i = j + 1; i < M; i++) {
          B[j * LDB + i] = B[i * LDB + j] * INV_A[i];
          B[i * LDB + j] = B[i * LDB + j] * INV_A[j];
        }
      }
    } else {
      // B is lower-stored.
      for (int j = 0; j < M; j++) {
        B[j * LDB + j] = B[j * LDB + j] * INV_A[j];
        for (int i = j + 1; i < M; i++) {
          B[i * LDB + j] = B[j * LDB + i] * INV_A[j];
          B[j * LDB + i] = B[j * LDB + i] * INV_A[i];
        }
      }
    }
  } else {
    ...
    if (UPPER_B) {
      ...
      for (int j = 0; j < M; j++) {
        B[j * LDB + j] = B[j * LDB + j] * INV_A[j];
        for (int i = j + 1; i < M; i++) {
          B[j * LDB + i] = B[i * LDB + j] * INV_A[j];
          B[i * LDB + j] = B[i * LDB + j] * INV_A[i];
        }
      }
    } else {
      /*
      ...
      for (int j = 0; j < M; j++) {
        B[j * LDB + j] = B[j * LDB + j] * INV_A[j];
        for (int i = j + 1; i < M; i++) {
          B[i * LDB + j] = B[j * LDB + i] * INV_A[i];
          B[j * LDB + i] = B[j * LDB + i] * INV_A[j];
        }
      }
    }
  }
