...
bool KernelSytrsv::tweakTransposition(Matrix& left, Matrix& right) const {
  const Matrix& rhs = left.isInverted() ? right : left;
  return rhs.isTransposed();
}

std::array<bool, 2U> KernelSytrsv::needsNewMatrix() const {
  return {false, false};
}

void KernelSytrsv::deduceName(const Matrix& left, const Matrix& right,
                              Matrix& result) const {
  const Matrix& rhs = left.isInverted() ? right : left;
...
string KernelSytrsv::generateCode(const Matrix& left, const Matrix& right,
  ...
  code += fmt::format("{}({}, {}, {}, {}, {}, {}, {}, {}, {}, {});\n",
                      CBLAS_DSYTRSV, LAYOUT, side, uplo_lhs, uplo_rhs, diag_rhs,
...
string KernelSytrsv::generateCost(const Matrix& left, const Matrix& right,
                                  const Matrix& result) const {
...
double KernelSytrsv::computeFLOPs(const Matrix& left, const Matrix& right,
                                  const Matrix& result) const {
  const double m = static_cast<double>(left.getNrows());
  return 7.0 / 3.0 * m * m * m;
}

std::vector<Variant> KernelSytrsv::getCoveredVariants() const {
...
string KernelSytrsv::infoInvocation(const Matrix& left, const Matrix& right,
                                    const Matrix& result) const {
  return Kernel::infoInvocation(CG_SYTRSV, left, right, result);
}

void KernelSytrsv::execute(const Matrix& _left, const Matrix& _right,
  ...
  cblas_dsytrsv(CblasColMajor, Side, UploA, UploB, DiagB, M, lhs.DATA,
                lhs.STRIDE, rhs.DATA, rhs.STRIDE);

  result = std::move(rhs);
}

void KernelSytrsv::loadModel() { model.read(getPathModel()); }

double KernelSytrsv::predictTime(const Matrix& left, const Matrix& right,
                                 const Matrix& result) const {
  uint8_t key = (0x00);
